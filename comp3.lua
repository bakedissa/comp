--// Fluent + SaveManager Setup //-- 
 ...
 local function taskManager() 
 	while taskAutomationEnabled do 
 		local success, err = pcall(function() 
 			...
 			-- Step 2: Find the highest-priority action and protect all matching quest slots 
 			local highestPriorityAction = nil 
 			local protectedSlots = {} 

 			-- Iterate through your quests in their priority order 
 				...
 				if toggle and toggle.Value then 
 					for _, task in ipairs(repeatableTasks) do 
 						...
 						if task.type == "Repeatable" and lowerTitle:find(questData.Pattern, 1, true) then 
 							-- A match is found. Protect its slot so it's not rerolled. 
 							if not protectedSlots[task.slot] then 
 								protectedSlots[task.slot] = true 
 							end 

 							-- If we haven't chosen an action yet, this is our highest-priority one. 
 			...
 			-- Step 3: Execute the single highest-priority action (if one was found) 
 			if highestPriorityAction then 
 				hatchEgg(highestPriorityAction.egg) -- This function now teleports
 			end 

 			-- Step 4: Reroll any repeatable slots that were NOT protected 
 		...
 		if not success then 
 			-- This is the only remaining 'warn' statement, left for critical error reporting. 
 			-- You can remove it if you want absolutely no output. 
 ...
 -- Removed the TweenSpeed slider as it's no longer needed
