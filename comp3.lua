...
task.spawn(function()
    while getgenv().Config.Enabled do
        -- ## NEW: Added proximity check ##
        -- Only opens boxes if the burst is active AND the player is near the main eggs.
        if isBurstActive and isPlayerInMainEggArea and isPlayerInMainEggArea() then
...
local Window = Fluent:CreateWindow({ 
	Title = "shitass comp script v3.1", -- updated version
...
local questToggles = {} 
local dontSkipMiningToggle -- Declare the toggle variable here to be accessible in taskManager

-- ## NEW: List of main egg positions for proximity check ##
...
local function taskManager() 
	while taskAutomationEnabled do 
		local success, err = pcall(function() 
            ...
            -- ## NEW: Protect Mining Egg quests if the toggle is enabled ##
...
-- ## NEW: Added toggle for "Don't Skip Mining Eggs" ##
