...
local MIN_ENEMY_RADIUS = 2 -- Minimum studs away from enemy
local MAX_ENEMY_RADIUS = 3 -- Maximum studs away from enemy
local TELEPORT_OFFSET = Vector3.new(0, 4, 0) -- Offset to avoid spawning in ground
local SAFE_AREA_CENTER = Vector3.new(-2, 23, 3) -- Middle area center
local SAFE_AREA_RADIUS = 50 -- Adjust based on your map size

-- Gamemode skipping config
local WANTED_GAMEMODES = { "FFA", "Teams", "Infection" }

local localPlayer = Players.LocalPlayer
local targetPlayer = nil
local connection = nil

-- Remotes and Values for Gamemode Skipping
...
local function getRandomPositionAroundTarget(targetPosition)
    ...
    local randomDistance = MIN_ENEMY_RADIUS + math.random() * (MAX_ENEMY_RADIUS - MIN_ENEMY_RADIUS)
...
local function getSafeTeleportPosition(targetPosition)
    ...
    return targetPosition + (directionToCenter * MAX_ENEMY_RADIUS) + TELEPORT_OFFSET
end

-- Gamemode skip logic
local function isGamemodeWanted()
    for _, mode in ipairs(WANTED_GAMEMODES) do
        if gameModeValue.Value == mode then
            return true
        end
    end
    return false
...
-- Gamemode Skip Handling
gameModeValue.Changed:Connect(handleGamemodeSkip)
task.wait(1) -- Wait a moment for initial value to be set
handleGamemodeSkip() -- Initial check
