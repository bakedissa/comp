...
local function teleportToTarget()
    ...
    -- Treat "Infected" as enemy regardless of same team
    local forceEnemyLogic = (localTeam and localTeam.Value == "Infected")

    if onSameTeam() and not forceEnemyLogic then
        -- Safe Teleport logic (phase-based like below offset)
        if not hasTriggeredBelowForThisLife then
            belowTimer = belowTimer + RunService.Heartbeat:Wait()
        end

        if belowTimer < BELOW_DELAY then
            -- Phase 1: Stay above teammate instead of colliding
            localRoot.CFrame = targetRoot.CFrame + TELEPORT_OFFSET
            print("[SAFE] Above teammate phase")
        else
            -- Phase 2: Move safely away (instead of kill)
            localRoot.CFrame = targetRoot.CFrame + Vector3.new(0, SAFE_TELEPORT_DISTANCE, 0)
            hasTriggeredBelowForThisLife = true
            print("[SAFE] Safe distance phase")
        end
    else
        -- Enemy logic (above then below)
        if not hasTriggeredBelowForThisLife then
            belowTimer = belowTimer + RunService.Heartbeat:Wait()
        end
