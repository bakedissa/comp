...
-- UI Elements
-- -------------------------
Tabs.AutoDeploy:AddParagraph({ Title = "Unit Deployment", Content = "Choose a unit and enable Auto Spawn. It will deploy, equip, and attack automatically." })
local unitDropdown = Tabs.AutoDeploy:AddDropdown("UnitToSpawn", { Title = "Unit to Spawn", Values = {"None"}, Default = "None" })
local autoSpawnToggle = Tabs.AutoDeploy:AddToggle("AutoSpawnEnabled", { Title = "Enable Auto Deploy & Attack", Default = false })

-- Backup Unit UI
Tabs.AutoDeploy:AddParagraph({ Title = "Backup Unit", Content = "If deploying the primary unit fails for a set time, it will switch to a backup." })
local useBackupToggle = Tabs.AutoDeploy:AddToggle("UseBackupUnit", { Title = "Enable Backup Unit", Default = false })
local backupUnitDropdown = Tabs.AutoDeploy:AddDropdown("BackupUnitToSpawn", { Title = "Backup Unit", Values = {"None"}, Default = "None" })
local backupDelayInput = Tabs.AutoDeploy:AddInput("BackupSpawnDelay", { Title = "Seconds to use Backup", Default = 10, Numeric = true, MaxLength = 3, Min = 1, Max = 120 })
...
-- Add a button to open the Discord link
...
local function rebuildUnitDropdown()
    ...
    if data then
        for name, v in pairs(data) do
            if type(v) == "table" and v.Type and (v.Type == "Unit" or v.Type == "Swarm") then
                if trophies >= (tonumber(v.Trophies) or 0) then
    ...
    if #filtered == 1 then filtered = {"None", "MiniPekka", "Giant Skeleton"} end
    
    unitDropdown:SetValues(filtered)
    backupUnitDropdown:SetValues(filtered)
    
    if not table.find(filtered, unitDropdown.Value) then
        unitDropdown:SetValue(filtered[2] or "None")
    end
    if not table.find(filtered, backupUnitDropdown.Value) then
        backupUnitDropdown:SetValue(filtered[2] or "None")
    end
end
...
local timeOfFirstDeployAttempt = nil 
local isUsingBackupUnit = false 

local function onCharacterAdded(char)
    local humanoid = char:WaitForChild("Humanoid")
    humanoid.Died:Connect(function()
        -- Player died, deployment will be re-attempted
    end)
    -- Reset backup unit state on respawn
    timeOfFirstDeployAttempt = nil
    isUsingBackupUnit = false
end
...
local function attemptToDeployUnit(unitNameToDeploy)
    ...
    -- This check is now here to avoid spamming
    ...
    pcall(function()
        ...
        local unitName = unitNameToDeploy -- Use the passed argument
        ...
        while not hasAnyTool() and attemptCount < 5 and Options.AutoSpawnEnabled.Value do
...
local function autoActionLoop()
    ...
    if equippedTool then
        -- STATE 1: Tool is equipped. ATTACK!
    ...
    elseif toolInBackpack then
        ...
        if Options.UseBackupUnit.Value and not isUsingBackupUnit and Options.UnitToSpawn.Value ~= "None" then
            if not timeOfFirstDeployAttempt then
                timeOfFirstDeployAttempt = os.clock()
            end

            local elapsedTime = os.clock() - timeOfFirstDeployAttempt
            local delay = tonumber(Options.BackupSpawnDelay.Value) or 10

            if elapsedTime > delay then
                isUsingBackupUnit = true
            end
        end
        
        if isUsingBackupUnit then
            unitToDeploy = Options.BackupUnitToSpawn.Value
        end
...
autoSpawnToggle:OnChanged(function(enabled)
    if enabled then
