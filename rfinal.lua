...
-- Attack conditions (FIXED for Infection mode)
local function isAttackable(target)
    ...
    local currentGamemode = gameModeValue.Value

    -- Handle FFA-like modes where everyone is an enemy.
    -- In these modes, team name might be "Survivor" or "FFA" but the logic is the same.
    local ffaModes = {
        FFA = true,
        LastStand = true,
        DuosLastStand = true,
        MultiJuggernauts = true
    }
    if ffaModes[currentGamemode] then
        return true
    end
    
    -- For all other modes (Teams, Infection, ColorWar, etc.), standard team logic applies.
    -- You can only attack players on a different team.
...
local function updateKillLoopState()
    ...
    if standardLoopShouldRun and not killConnection then
        ...
        debugAction("system", "Standard Kill Loop started")
    elseif not standardLoopShouldRun and killConnection then
        killConnection:Disconnect()
        killConnection = nil
        debugAction("system", "Standard Kill Loop stopped")
    end
...
Fluent:Notify({ Title = "Utility GUI", Content = "Infection targeting fixed. Loaded successfully.", Duration = 6 })
