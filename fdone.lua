...
Notes: Combines the reliable auto-kill system from the original script with the improved boundary-safety, SkipShieldUsers option, UI layout, and other improvements from the patched script. Added Auto Upgrade Equipped Weapon + Unarmed hug logic + AFK prioritization. ]] 
...
-- Find target player helper (by name) and hook respawn
local function findTargetPlayer(username)
    ...
    if targetPlayer then
        ...
        -- Reset teleport state when they respawn
...
-- Attempt to pick a sensible teleport target automatically (nearest eligible)
local function findNearestValidTeleportTarget()
    local best = nil
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= localPlayer and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if aliveVal and aliveVal.Value == true then
                -- re-use isAttackable logic below (forward declare later), so we'll call a simple check here:
                -- We'll just ensure target has Stats & team for teleport selection, and is alive.
                if p:FindFirstChild("Stats") and p.Stats:FindFirstChild("Team") then
...
local function startTeleporting()
    ...
    if not Options.TeleportEnabled or not Options.TargetPlayer or Options.TargetPlayer.Value == "None" then
        -- If no explicit selected target, try picking a nearest valid target automatically
        if Options.TeleportEnabled and (not Options.TargetPlayer or Options.TargetPlayer.Value == "None") then
            local autoTarget = findNearestValidTeleportTarget()
            if autoTarget then
                targetPlayer = autoTarget
                debugAction("target", "Auto-teleport target chosen: " .. targetPlayer.Name)
            else
                return
            end
        else
        ...
        if not findTargetPlayer(Options.TargetPlayer.Value) then return end
    end

    heartbeatConnection = RunService.Heartbeat:Connect(function()
        pcall(teleportToTarget)
    end)
    debugAction("system", "Teleport loop started for " .. (targetPlayer and targetPlayer.Name or "None"))
end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player.Character and player:FindFirstChild("Stats") and player.Stats:FindFirstChild("Weapon") then
            ...
            if playerCharacter then
                ...
                local hitboxPart = weaponModel and ( weaponModel:FindFirstChild("Hitbox") or weaponModel:FindFirstChild("WeaponHitbox") or weaponModel:FindFirstChild("WeaponHitBox") or weaponModel:FindFirstChild("Part") )
                if hitboxPart then
...
local function isAttackable(target)
    ...
    -- Special-case: Infection gamemode pairing rules
    if gameModeValue and gameModeValue.Value == "Infection" then
        -- Survivors only attack Infected, Infected only attack Survivors.
        if localTeam == "Survivor" and targetTeam == "Infected" then return true end
        if localTeam == "Infected" and targetTeam == "Survivor" then return true end
        return false
    end

    -- If either is FFA, allow attacking (fallback)
    if targetTeam == "FFA" or localTeam == "FFA" then return true end

    -- Fallback: standard team-opposite check
...
-- Chaotic Auto Kill
-- ========================================================================
-- When enabled, chooses random eligible enemy and rapidly teleports between them
local function startChaoticAutoKill()
    if chaoticConnection then return end
    chaoticConnection = RunService.Heartbeat:Connect(function()
        -- We'll run a fast teleport loop asynchronously using task.spawn (keeps Heartbeat responsive)
        task.spawn(function()
            -- do rapid picks until disabled
            while Options.ChaoticAutoKill and Options.ChaoticAutoKill.Value and isAlive do
                ...
                if #pool == 0 then
                    ...
                    -- teleport near them safely
                    local localRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
                    local targetRoot = pick.Character and pick.Character:FindFirstChild("HumanoidRootPart")
                    if localRoot and targetRoot then
                        -- Respect same-team/juggernaut safety like normal teleport
                        if onSameTeam(localPlayer, pick) or isJuggernaut() then
                            if not hasTeleportedThisLife then
                                localRoot.CFrame = targetRoot.CFrame + Vector3.new(0, -50, 0)
                                hasTeleportedThisLife = true
                            end
                        else
                            local tpPos = getSafeTeleportPosition(targetRoot.Position)
                            tpPos = clampInsideBounds(tpPos)
                            pcall(function() localRoot.CFrame = CFrame.new(tpPos) end)
                        end
                    end
                    task.wait(0.05) -- rapid switch delay
                end
            end
        end)
        -- stop this heartbeat driver if ChaoticAutoKill disabled
        if not (Options.ChaoticAutoKill and Options.ChaoticAutoKill.Value) then
            if chaoticConnection then
                chaoticConnection:Disconnect()
                chaoticConnection = nil
            end
        end
    end)
    debugAction("chaos", "Chaotic Auto Kill started")
end

local function stopChaoticAutoKill()
    if chaoticConnection then
        chaoticConnection:Disconnect()
        chaoticConnection = nil
        debugAction("chaos", "Chaotic Auto Kill stopped")
    end
end
...
-- Chaotic Auto Kill toggle
Tabs.Combat:AddParagraph({ Title = "Chaotic Auto Kill", Content = "When enabled, rapidly teleports between random eligible enemies." })
local chaoticToggle = Tabs.Combat:AddToggle("ChaoticAutoKill", { Title = "Chaotic Auto Kill", Default = false })
...
Players.PlayerRemoving:Connect(function(player)
    updatePlayerLists()
    -- Teleport auto-retarget: if the teleport target leaves, try to auto-pick a nearby valid target
    if player == targetPlayer then
        debugAction("target", "Teleport target left: " .. tostring(player and player.Name))
        -- Try to pick a nearest valid teleport target
        local newTarget = findNearestValidTeleportTarget()
        if newTarget then
            targetPlayer = newTarget
            debugAction("target", "Auto-retargeting teleport to: " .. newTarget.Name)
            -- ensure teleport loop active
            if Options.TeleportEnabled and Options.TeleportEnabled.Value then
                ...
                debugAction("system", "Teleport loop restarted for auto-target " .. newTarget.Name)
            end
        else
            stopTeleporting()
        end
    end
    -- If the auto-kill current target leaves, reset currentTarget so we pick a new one immediately
    if player == currentTarget then
        debugAction("kill", "Current kill target left: " .. tostring(player and player.Name))
        currentTarget = nil
    end
end)
...
local function hookAlive()
    ...
    aliveValue:GetPropertyChangedSignal("Value"):Connect(function()
        ...
        if isAlive then
            ...
            -- when you die, reset targeting (so it doesn't try to chase dead/invalid players).
            stopTeleporting()
            stopAFKScanner()
            currentTarget = nil
            stopChaoticAutoKill()
...
teleportToggle:OnChanged(function(enabled)
    if enabled then startTeleporting() else stopTeleporting() end
end)
playerDropdown:OnChanged(function()
    if Options.TeleportEnabled and Options.TeleportEnabled.Value then startTeleporting() end
end)
...
local function updateKillLoopState()
    ...
    local chaoticEnabled = Options.ChaoticAutoKill and Options.ChaoticAutoKill.Value

    -- Chaotic overrides normal kill loop when enabled
    if chaoticEnabled then
        -- stop normal kill loop if running
        if killConnection then
            killConnection:Disconnect()
            killConnection = nil
            debugAction("system", "Kill loop stopped due to chaotic mode")
        end
        startChaoticAutoKill()
        return
    else
        -- ensure chaotic stopped
        stopChaoticAutoKill()
    end
...
chaoticToggle:OnChanged(updateKillLoopState)
