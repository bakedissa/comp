...
local function teleportToTarget()
    ...
    if onSameTeam(localPlayer, targetPlayer) or isJuggernaut() then
        ...
        -- prefer safe random offset (original behavior)
...
-- Attack conditions including Skip Shield Users option
...
local function killLoop()
    ...
    if weaponPart and targetRoot then
        ...
        -- safe push-in: compute desired position behind target, then clamp inside bounds
        local desiredPos = targetPosition - (targetRoot.CFrame.LookVector * weaponRange)
        desiredPos = clampInsideBounds(desiredPos)

        -- maintain a little vertical offset to avoid ground clipping
        -- lock Y to target's height so you stay grounded
        local finalPos = Vector3.new(desiredPos.X, targetRoot.Position.Y, desiredPos.Z)
...
Tabs.Automation:AddParagraph({ Title = "Gamemode Skipper", Content = "Automatically skip undesirable game modes." })
...
autoSkipToggle:OnChanged(function()
    -- immediate check when toggled on
