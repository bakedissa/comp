...
local function chaoticKillLoop()
    ...
    if tgtRoot then
        ...
        localRoot.CFrame = CFrame.new(
            Vector3.new(desiredPos.X, localRoot.Position.Y, desiredPos.Z),
            tgtRoot.Position
        )
    end
end

local function updateChaoticLoopState()
...
local chaoticKillToggle = Tabs.Combat:AddToggle("ChaoticAutoKill", {
    Title = "Chaotic Auto Kill (Random Fast Teleport)",
    Default = false
})
chaoticKillToggle:OnChanged(updateChaoticLoopState)
