...
local function teleportToTarget()
    ...
    if onSameTeam() then
        ...
        -- ENEMY TEAM / FFA: Lock onto a random spot within map bounds

        -- Step 1: If we don't have a locked position for this life, create one.
        if not lockedEnemyPosition then
            -- Calculate a random horizontal position around the target
            ...
            local initialPosition = Vector3.new(
                ...
                targetRoot.Position.Y - 3, -- Use the target's Y-level with a -3 offset
                targetRoot.Position.Z + offsetZ
            )

            -- Clamp the final position to the specified map bounds
            local clampedX = math.clamp(initialPosition.X, -28, 29)
            local clampedZ = math.clamp(initialPosition.Z, -27, 31)

            local finalPosition = Vector3.new(
                clampedX,
                initialPosition.Y, -- Keep the calculated Y
                clampedZ
            )

            lockedEnemyPosition = CFrame.new(finalPosition)
            debugAction("teleport", "Enemy team. Locked new random position, clamped to map bounds.")
        end
        
        -- Step 2: Continuously teleport to the locked position.
