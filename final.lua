--[[ 
Title: Teleport & Utility GUI (Merged + Chaotic Kill)
Description: Unified Teleport, Combat, and Utility GUI using Fluent with Precise Targeting, original kill, and new Chaotic Kill mode.
Author: Gemini (patched)
Notes: Combines reliable auto-kill, improved boundary-safety, SkipShieldUsers, AFK prioritization, UI layout, Chaotic Kill toggle with height/delay.
]]
...
local AFKPlayers = {}
local lastPositions = {}
local DEFAULT_AFK_TIME = 2
local DEFAULT_AFK_THRESHOLD = 0.2
...
-- ========================================================================
-- Core kill loop (Precise Targeting / original kill)
-- ========================================================================
...
-- Chaotic Kill logic
-- ========================================================================
local function startChaoticKill()
    stopChaoticKill()
    chaoticConnection = RunService.Heartbeat:Connect(function()
        if not isAlive or not Options.ChaoticKill.Value then return end
        local localRoot = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
        if not localRoot then return end
        ...
        local targets = {}
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= localPlayer and isAttackable(plr) and plr.Character and plr.Character:FindFirstChild("HumanoidRootPart") then
        ...
        local yHeight = tonumber(Options.ChaoticYHeight.Value) or 4
        local teleportPos = Vector3.new(targetRoot.Position.X, yHeight, targetRoot.Position.Z)
        localRoot.CFrame = CFrame.new(teleportPos, targetRoot.Position)
        task.wait(tonumber(Options.ChaoticDelay.Value) or 0.05)
    end)
end

function stopChaoticKill()
...
chaoticKillToggle = Tabs.Combat:AddToggle("ChaoticKill", { Title = "Enable Chaotic Kill", Default = false })
chaoticKillToggle:OnChanged(function(enabled)
    if enabled then
        stopTeleporting()
        stopAFKScanner()
        startChaoticKill()
        ...
        if isAlive then
            startAFKScanner()
            if Options.KillAllPlayers.Value or Options.KillPlayers.Value then
                updateKillLoopState()
            end
        end
    end
...
chaoticHeightInput = Tabs.Combat:AddInput("ChaoticYHeight", { Title = "Teleport Y Height", Default = "4", Numeric = true, Finished = true })
chaoticDelayInput = Tabs.Combat:AddInput("ChaoticDelay", { Title = "Delay Between Teleports (s)", Default = "0.05", Numeric = true, Finished = true })
...
Window:SelectTab(1)
Fluent:Notify({ Title = "Utility GUI", Content = "Script loaded successfully!", Duration = 4 })
